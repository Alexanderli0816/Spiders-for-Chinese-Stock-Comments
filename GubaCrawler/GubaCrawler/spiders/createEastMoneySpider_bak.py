...
def search_dict(key1, dictionary):
    for key in dictionary.keys():
        if key == key1:
            return 0, dictionary[key1]
        else:
            if isinstance(dictionary[key], dict):
                status, dict_result = search_dict(key1, dictionary[key])
                if status == 0:
...
class EastMoneySpider(scrapy.Spider):
    name = 'createEastMoneySpider_bak'
    ...
    lua_script = \
        '''function main(splash, args)
            ...
            if string.find(request.url, ".aspx") ~= nil 
    ...
    def parse(self, response):
        ...
        if len(selector.extract()) > 0:
            ...
            for line in selector:
                ...
                if '万' in readNum:
                    request.meta['readNum'] = int(readNum[:-1]) * 10000
                else:
    ...
    def parse_comments(self, response):
        ...
        if reply_page == 1:
            ...
            try:
                ...
                Guba_info['commentWriter'] = data_json['user_nickname']
                ...
                Guba_info['commentWriterID'] = None
                Guba_info['commentWriter'] = None
                Guba_info['writerInfluence'] = None
                Guba_info['writerUserAge'] = None
                Guba_info['writerIsMajia'] = None
                Guba_info['writerIsBackType'] = None
        ...
        selector_reply = response.xpath('//*[@id="zwlist"]//div[@class="level1_item clearfix" and @data-reply_id]')
